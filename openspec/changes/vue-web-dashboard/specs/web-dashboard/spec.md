...
### Requirement: 实景动态主视觉
首页 SHALL 采用大工蓝、墨色、纸白的 Naive UI 主题，以全屏校园实景作为首屏背景。首页 SHALL 不使用现有游戏画面或其他游戏美术。当前没有适合的校园视频时，SHALL 使用真实照片的缓慢平移与缩放，不冒充实拍视频。

#### Scenario: 动效控制
- **WHEN** 用户点击暂停背景动效或系统偏好减少动态效果
- **THEN** 背景停止平移缩放，所有介绍与下载功能仍可用
