...
- 首次启动显示 Godot 封面与邮箱、密码登录表单，直接通过 PocketBase 认证；手动登录或恢复登录后停留主菜单，点击“进入游戏”才加载校园并连接，游戏服确认后进入；正常进入后不再显示登录或继续按钮，普通断线返回主菜单并保留有效登录，点击“进入游戏”手动重连；认证失效时返回登录封面，传送直接回到游戏。暂停/失焦停止行走，点击世界或 Escape 恢复。操作与数据精度写入文档。
