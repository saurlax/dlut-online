...
`build-web.yml` 与 `build-game.yml` 各自按路径接收分支 push、PR 和手动运行；内部前置 `test` 调用共享 `test.yml`，成功后才执行 `build`。轻量测试使用 Linux runner，只执行 Vue `vue-tsc --noEmit`、Go `go test -timeout 60s ./...` 与 `go vet ./...`，不拉取 LFS 资产或安装 Godot。两端同时构建时各自运行一次轻量测试。版本标签发布先校验版本，再复用两个构建工作流各自执行 test → build，全部成功后下载本次构建的 ZIP 并上传 GitHub Release，不直接复用历史构建运行的产物。
