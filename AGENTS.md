...
- 游戏协议版本 4 使用 ENet/UDP，玩家 ID 为 15 字节 ASCII：账号使用 PocketBase 小写 ID，游客使用大写十六进制进程 ID。控制消息可靠传输，输入与二进制快照使用不可靠有序通道；通过序号、server_tick 和 map_epoch 拒绝迟到状态。客户端可达的 `enet://` 或 `enets://` 端点存入 PocketBase `game_servers` Collection，由票据接口动态下发；生产只接受 `enets://`。游戏服使用 `DO_GAME_SERVER_PORT` 监听 UDP，生产使用 DTLS 证书，客户端校验证书。
