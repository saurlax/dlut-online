...
- 首次启动显示 Godot 封面与“单人模式”“多人模式”入口，不执行账号恢复。单人模式纯本地加载校园，不连接 API 或游戏服；大地图左上角可以修改时间和天气，设置跨校区保留。多人模式点击后，未登录时弹出邮箱、密码登录弹窗并尝试恢复凭据；认证成功或已登录时直接加载校园并连接，游戏服确认后进入。普通断线返回首页并保留有效登录，点击“多人模式”手动重连；认证失效时清除会话并返回首页。传送直接回到游戏。暂停/失焦停止行走，点击世界或 Escape 恢复。操作与数据精度写入文档。
