...
#### Scenario: Eda photo-supported banks
- **WHEN** 生成开发区思源湖和涌泉湖的水面与周边地形
- **THEN** 按学校航拍和近景确认的岸高水低关系建立草岸缓坡，水面低于岸上步道；保留有效水面平面来源；开发区允许按照片连续岸线特征，在原线段2米走廊内作有界曲线细化并保留原始节点，近岸 16 米内局部修正并连续接回原 DSM，地形、路面、植被和权威碰撞共同重新生成。岸高、坡宽和水位只记为视觉近似，不能冒称实测。
