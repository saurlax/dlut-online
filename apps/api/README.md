...
客户端使用 Godot 原生邮箱和密码表单调用 `POST /api/collections/users/auth-with-password`，请求包含 identity（邮箱）和 password。PocketBase AuthRule 要求邮箱已验证且账号未禁用，客户端首页提供单人模式与多人模式；单人模式不访问 API，点击多人模式后，未登录时弹出登录表单，收到 token 和 record 后直接加载校园并申请游戏票据。密码隐藏，提交时清空输入框，不持久保存或输出日志；Token 按 API 根地址隔离保存在 macOS 钥匙串或 Windows 凭据管理器，密码不保存。首次点击多人模式时读取 Token 并调用 `POST /api/collections/users/auth-refresh`，验证通过建立会话并继续连接游戏服。PocketBase 0.40.3 使用同一个有效 auth token 刷新并签发新 token，没有独立 refresh token；过期 token 无法续期，需重新输入密码。取消终止请求并忽略迟到响应，错误后可以重新输入并登录。
...
每个校园场景实例化 `scenes/campus_environment.tscn`，静态环境可在编辑器查看。程序化体积云层在世界空间 1200 至 2100 米高度使用三维噪声密度场，半分辨率下每视线最多 32 步积分、每个非空采样向太阳追加两次密度探测实现内部遮光，低透射率提前终止。云高和厚度为视觉参数，非天气 API 实测云底高度；没有云层投射到建筑地面的精细动态阴影。4 Hz 更新环境参数，不增加透明云网格或额外实时灯光。云量、风、阴暗程度约 20 秒平滑变化；太阳角度按真实日期计算。雨雪按现有 WMO 天气代码映射视觉强度，毛毛雨、雨和冻雨使用雨丝，雪、米雪和阵雪使用雪花，雷暴沿用强雨效果；未知代码不触发降水。雾天代码 45/48 独立控制浓雾和天空雾化，雨雪使用较轻距离雾，约 20 秒平滑过渡；这些强度及雾密度是视觉参数，不是实测降水量或能见度。无新增探索 HUD 和中文运行时文案，无需更新字体子集。天气来源与许可见根目录 `CREDITS.md`，查询坐标和精度约定见 `references/shared/weather.md`，桌面导出从仓库根目录读取并内嵌 `CREDITS.md`，macOS DMG 根目录同时附带该声明。

降水场景为 `scenes/campus_precipitation.tscn`，已静态挂载到共享环境。雨最多 6000 个粒子、雪最多 4500 个粒子，使用世界空间 GPU 模拟和共享程序化软边材质，随相机发射，24 米外淡出；风向沿用地图 X 向东、Z 向南约定，风速对雨雪的影响使用视觉缩放。晴天停止发射并隐藏降水，数据过期时随天气回退消退。暂停和失焦时天气继续运行，场景切换释放旧粒子，大距离移动重置粒子。

降水遮挡使用已有可见网格生成的 96 × 96 米局部高度场，垂直范围 256 米、分辨率 512，位置按 4 米网格更新，雨雪接触最上层屋顶或地面即隐藏。该高度场不改变玩家碰撞，也不引入新建筑或室内；不精确表达多层室内、侧向风雨、细小屋檐和树冠孔隙。未实现积雪、结冰、闪电、雨声及地面湿润材质。新增资源随共享场景依赖进入 Windows/macOS 导出，不进入游戏服资源包；API、协议、环境变量和 migration 无变化。

### 客户端手动入场与断线
首页选择单人模式直接本地加载校园，不访问账号或游戏服务；多人模式已登录则直接连接，未登录弹出原生表单并尝试恢复，认证成功后继续加载默认校园、申请票据并连接。单人模式的大地图左上角提供固定 UTC+8 时刻（分钟精度）与天气设置，跨校区保留、重启恢复默认 12:00 晴天。普通断线卸载校园，保留有效登录并等待手动重连；认证失效仍需重新登录。连续镜头转向约 20 Hz 同步，方向键、停步、奔跑和跳跃边沿即时发送；游戏服每连接最多 80 条消息/秒。ENet 协议仍为 5，关闭码 4002 仅表示版本不兼容，4005 表示消息限流，4006 表示通信数据异常；新客户端与游戏服应配套更新以显示准确原因。无新增环境变量或数据库迁移。
