...
- 正式客户端支持 Windows x86_64 和 macOS arm64，另提供 Android arm64 测试 APK，共用 Godot 项目；不支持或导出 Web 游戏。Android 提供触屏移动、环视、跳跃、暂停和地图操作，真机性能与联网体验仍需验收。
- 当前建设可行走的校园世界、PocketBase 账号和 ENet 玩家同步，多人模式玩家必须使用真实账号，单人模式纯本地且无需账号；室内必须先具备实景照片或有效平面数据再建设，不自行增加活动、战斗、任务或奖励功能。
...
- 首次启动显示底部进度条加载页，校园背景加载并渲染就绪后显示 Godot 封面与“单人模式”“多人模式”入口，不执行账号恢复。单人模式纯本地加载校园，不连接 API 或游戏服；大地图左上角可以修改时间和天气，设置跨校区保留。多人模式点击后，未登录时弹出邮箱、密码登录弹窗并尝试恢复凭据；认证成功或已登录时直接加载校园并连接，游戏服确认后进入。普通断线返回首页并保留有效登录，点击“多人模式”手动重连；认证失效时清除会话并返回首页。传送直接回到游戏。暂停/失焦停止行走，点击世界或 Escape 恢复。操作与数据精度写入文档。
...
- 建筑、道路、广场、水域、绿地等几何与外观证据的可信优先级为 **照片 > 官网俯视图 > OSM**。发生冲突时，优先采用照片能确认的内容，其次采用俯视图；OSM 作为统一坐标底稿和证据不足时的基础数据，不能为了贴合 OSM 而否定更可信的可见特征。每项判断记录照片日期、视角、遮挡、配准依据与不确定性；照片透视外包络、屋檐外沿和官网斜视交互 bound 均不能直接充当地面墙脚，俯视绘图也不是测绘真值。无法判定的冲突保留待核对状态，不自动平均。统一使用 WGS84 与同一校区原点生成局部 X 向东、Z 向南坐标，禁止按要素类型分别叠加旧官网质心平移。保留官方 Feature ID、OSM 对象 ID 及原始档案，建筑模型独立可替换；记录转换、适用范围与误差，不将拟合残差声称为实测精度。
- 地形平面要素同样以 OSM 为基础；OSM 不包含完整连续高程时保留已有有来源的 DSM/DEM，并直接对齐相同 WGS84 地理基准，不将其误标为 OSM 高程。建筑、路面、广场、植被、室外台阶和碰撞在基准变更后必须重新生成与核对，不能只移动可见模型。
...
- 三校区统一使用 OSM 平面基准和有证据的官网俯视图局部修正；保留已核对的照片立面，具体匹配、暂缺数据和精度边界记入各校区来源记录。旧官网轮廓模型需要迁移，不作为新的基底依据。
...
- 提交前检查 git status、diff 和暂存文件清单，逐项确认与当前需求相关；按明确文件路径暂存，不使用 git add . 混入无关内容。用户已授权每次任务完成并通过相关检查后及时创建提交；代码按独立、可审查的任务及时提交，资源按下条规则集中提交，不混入他人的未完成工作。
- 代码提交与资源提交分离。涉及 Git LFS 文件修改时，同一任务或同一批资源的反复调整、生成与验证尽量在工作区完成，稳定后合并为尽量少的资源提交，不为每次中间修改单独提交，以减少不同内容版本产生的 LFS 对象。代码仍可正常及时提交；交付前集中提交对应的最终资源并核对代码与资源一致。LFS 按内容存储，合并 commit 不等于自动删除已经生成或上传的旧 LFS 对象；此规则不授权改写已共享历史或清理远端对象。
...
- build.yml 统一接收 main 源码分支 push、PR、手动运行，也提供 workflow_call 供发布复用；纯文档变更不触发；根目录 CREDITS.md 作为发行资源参与构建触发。其他分支 push 不触发，避免 PR 重复运行。每次调用先执行 test.yml，成功后按路径筛选结果并行执行 windows、macos、server、web、android 中相应的独立构建任务，五者必须依赖 changes 和 test，按路径选择时也不能跳过测试门禁。Windows 导出内嵌资源的 x86_64 独立 EXE；macOS 在 macOS runner 导出、裁剪为 arm64 并重新做 ad-hoc 签名，最终 DMG 必须通过严格签名与 arm64 架构校验；server 在 Linux 生成碰撞世界、导出游戏服并构建镜像；web 构建 Go HTTP 镜像。Android 在 Linux 使用 JDK 17、Android SDK 和 Godot 模板导出 arm64 Release APK，从仓库 Actions Secret `DO_ANDROID_KEYSTORE_BASE64` 恢复固定 debug keystore，上传 APK artifact 并在摘要提供下载链接，版本发布时另作为 GitHub Release 附件供官网下载，不提交密钥或将其放入 artifact/cache；常规构建缺失 Secret 时失败，外部 fork/Dependabot PR 使用临时测试签名并在摘要明确提示。本机复用 `~/.android/debug.keystore`，缺失时按标准参数初始化；固定签名版本可覆盖更新，签名不同时需卸载旧版。普通 PR/main push 按应用路径选择构建：apps/web/ 或 apps/api/ 改动构建 web，apps/game/ 改动构建 windows、macos、android、server；跨应用变更取并集。CREDITS.md、工作流及 CI 脚本、.dockerignore、.gitattributes、compose.yaml 改动触发全量构建。版本发布、手动运行和无法确定变更基线的新分支 push 全量构建；仍保留一个 build.yml，不增加 ci.yml 调度层。
- release.yml 仅接收版本标签，先校验版本，再调用 build.yml（test → 五种构建），全部成功后下载本次构建的 EXE/DMG/APK 上传 GitHub Release。测试失败必须阻止所有 build 和 release，不能使用 always()、continue-on-error 或跳过测试参数绕过。构建阶段只做必要生成、编译、打包、产物完整性检查和上传，不运行集成/E2E，也不构建 Godot Web 产物。Go `DO_API_SERVER_PORT` 默认 8415，并兼容平台 `PORT`；容器 PocketBase 数据位于 `/data/pb_data` 持久卷，Docker 构建上下文为根目录；Windows 暂不签名，macOS 未配置 Developer ID 和公证，须如实说明 Gatekeeper 限制。
...
- 客户端使用 Godot 原生邮箱、密码表单调用 PocketBase 密码认证，不打开本机回调端口。密码输入隐藏，提交后清空，不写入文件或日志；账号 token 安全保存到系统凭据库，首次点击多人模式时通过 PocketBase auth-refresh 验证并刷新，认证失效或退出时清除；网络暂时故障不删除有效凭据。无账号提示去 dlut.online 注册。未来 OIDC 与移动系统认证另行实现。

- DLUTMap 的交互 bound 已确认为三维斜视选择框，禁止用于任何地面几何、兜底轮廓、地形基台、植被定位、地图或碰撞。仅保留原始档案与来源 ID；缺少独立有效地面轮廓时保留空参考节点，待配准后再生成，不能平均或矫正旧框继续使用。官网俯视图与实景照片仍可作为独立证据。
