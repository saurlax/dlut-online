...
- **THEN** 身份输出保留候选及geometry_deferred标记，凌水准备工具保留待替换几何，不因名称或ref对应成功而自动迁移；待核对状态不得计入已应用来源几何
...
- **THEN** 显式登记共享楼群，整体来源几何只生成一次；保留所有官方ID、原始轮廓和可替换节点，未分区身份引用几何所有者
...
经逐项来源审查的非建筑统计点 SHALL 保留原始身份与轮廓，但 MUST NOT 作为缺省估计楼体挤出；分类覆盖必须有显式来源配置。

#### Scenario: 统计点被官方通用类型误标为建筑
- **WHEN** 单项官方详情和图面核对证明该对象是统计或交互参考区域，缺乏独立建筑依据
- **THEN** 以显式审查配置保留ID、原始轮廓及可替换空节点，不挤出估计楼体，不生成建筑基台或碰撞
