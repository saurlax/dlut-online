...
func run() -> void:
	...
	for server in [false,true]:
		...
		for sample in [[Vector2(-260,105),20.18],[Vector2(-263.613,85.606),16.225]]:
		...
		var wall := space.intersect_ray(PhysicsRayQueryParameters3D.create(Vector3(-260,1.7,140),Vector3(-260,1.7,105)))
		assert(not wall.is_empty(),"Exterior must remain closed")
		world.free()
		viewport.free()
	print("PASS: client/server comprehensive roof, lower glass projection and closed exterior")
	quit()
