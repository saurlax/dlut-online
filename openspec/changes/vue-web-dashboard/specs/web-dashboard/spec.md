## ADDED Requirements

### Requirement: Vue 网站
网站 SHALL 使用 Vue 3、TypeScript、Vite 和 Naive UI，通过 Go 嵌入静态构建产物并同域提供 API，运行时不依赖 Node。Naive UI SHALL 提供统一主题及可复用的按钮、状态、卡片和数据展示组件，为后续账户管理与数据工具复用。

#### Scenario: 访问网站
- **WHEN** 用户打开 Go Web 首页
- **THEN** 显示产品介绍、客户端下载入口、总在线人数及三校区状态，并适配移动屏幕

#### Scenario: 未知资源
- **WHEN** 请求不存在的静态资源、API 或旧游戏 Web 路由
- **THEN** 返回 404 而非首页 HTML

### Requirement: 在线状态时效
网页 SHALL 每 10 秒请求公开在线接口，展示更新时间；失败、未上报或超过 15 秒的数据 SHALL 显示状态未知，不以零或历史人数冒充当前人数。

#### Scenario: 正常在线
- **WHEN** 接口返回有效 live 状态
- **THEN** 展示真实总人数、三校区人数及更新时间

#### Scenario: 断连或数据过期
- **WHEN** HTTP 请求失败或上报数据失效
- **THEN** 清楚显示状态未知并保留可用的最后更新时间，后续请求成功自动恢复

### Requirement: 首页设计语言
首页 SHALL 使用大工蓝、纸白与墨色的统一 Naive UI 主题，采用宋体主标题、校园主视觉、清晰下载入口与紧凑在线信息。校园照片 SHALL 标注实景，不冒充游戏截图。

#### Scenario: 桌面访问首页
- **WHEN** 用户打开首页
- **THEN** 显示 DLUT Online 品牌、校园主题首屏、客户端下载与 GitHub 入口，在线数据沿用既有时效语义

#### Scenario: 小屏和键盘访问
- **WHEN** 用户在小屏浏览或使用键盘操作
- **THEN** 页面无横向溢出，入口有清晰焦点，信息按阅读顺序排列，减少动态效果偏好得到尊重
