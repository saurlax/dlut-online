"""Crop pinned Copernicus DSM samples without resampling (Python 3 + GDAL CLI)."""
...
def build(campus, config, cache, output):
    ...
    info = json.loads(run("gdalinfo", "-json", tile))
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        cropped = json.loads(run("gdalinfo", "-json", crop))
    ...
    source = {
        ...
        "source": spec, "retrieved": config["retrieved"],
        ...
        "model_alignment": "未核实官方校园轮廓水平基准，禁止直接作为现有场景坐标使用。",
        "use": "离线地形基础资料，未接入客户端或碰撞世界。",
        ...
        "processing_software": run("gdalinfo", "--version").strip(),
    }
...
def main():
    ...
    parser.add_argument("--campus", choices=["lingshui", "eda", "all"], default="all")
    ...
    for campus in config["campuses"] if args.campus == "all" else [args.campus]:
        build(campus, config, args.cache.resolve(), args.output_root.resolve() / campus / "terrain")
