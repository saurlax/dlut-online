...
class DesktopExport extends EditorExportPlugin:
	...
	func _export_begin(features: PackedStringArray, _is_debug: bool, _path: String, _flags: int) -> void:
		...
		if config.is_empty():
			...
			return
		var credits_path := ProjectSettings.globalize_path("res://").path_join("../../CREDITS.md").simplify_path()
		var credits := FileAccess.get_file_as_bytes(credits_path)
		if credits.is_empty():
			get_export_platform().add_message(EditorExportPlatform.EXPORT_MESSAGE_ERROR,
				"Third-party notices", "Cannot read repository CREDITS.md")
			return
		add_file("res://CREDITS.md", credits, false)
