...
apps/web 保留当前 Go 源码布局，负责站点及 HTTP API；程序仍构建为 server，现有 Go 模块名保持兼容，不因目录名称增加 package 分层。
...
游戏服镜像命名为 dlut-online，Go HTTP 镜像命名为 dlut-online-web；CI 本地标签、GHCR 发布与 Release 说明同步，版本标签规则保持不变。
