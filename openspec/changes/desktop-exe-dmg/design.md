...
工作流固定为 test.yml、build.yml、release.yml。build.yml 先调用 test.yml，成功后按应用变更选择 windows、macos、server、web、android 五个独立 job；前端/API 仅构建 web，游戏构建三个客户端和 server，共享构建文件、手动与发布全量构建；Windows 与服务器运行于 Linux，macOS 运行于 macOS。Release 校验版本后复用整个 build.yml，必须等待测试和五个构建全部成功，明确指定 EXE、DMG 与 Android 测试 APK 三个附件，不上传 ZIP。Actions 自身的 artifact 下载封装不属于发布附件格式。
