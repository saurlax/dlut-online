...
#### Scenario: Rendering offsets do not become unsupported curbs
- **WHEN** 基于共同地形生成道路和地面覆盖层
- **THEN** 仅保留薄层渲染间隙，普通行走能够双向跨越开阔道路边缘，不以跳跃绕过渲染抬升造成的阻挡
- **AND** 显式关闭碰撞的装饰边条在地形拟合后仍不参与碰撞，保存服务端与客户端形状一致；此检查不代表未知真实路沿或台阶已测量
