...
main.tscn 仅挂载原生登录封面，不引用校园 PackedScene。封面复用 campus_hud 的登录控件，但无玩家时不创建地图、读取校园数据或生成碰撞。PocketBase 密码认证或 auth-refresh 成功后停留主菜单，玩家点击“进入游戏”才使用 ResourceLoader 线程加载默认凌水场景，完成切换后再申请游戏票据；游戏服 welcome 仍是允许行走的必要条件。加载中禁用重复提交，失败保留账号会话并允许重试。退出或认证失效切回轻量 main.tscn，卸载原校园；校区传送维持现有连接与加载流程。

## 主菜单与连接生命周期
手动认证和保存会话刷新只建立账号会话，不触发校园加载或取票。主菜单显示账号和“进入游戏”，点击后设置一次性入场意图，再异步加载默认校园并连接。正常传送仍复用已有连接，不出现主菜单。
