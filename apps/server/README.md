...
Go `PORT` 默认 8060，`-addr` 优先；启动不需要 Web 导出目录。另开客户端使用 `godot --path apps/client`，不自动加载 `.env`。
...
```sh
mkdir -p apps/client/build/server apps/client/build/windows apps/client/build/macos
godot --headless --path apps/client --script tools/server_export/build_worlds.gd
godot --headless --path apps/client --export-release Server build/server/dlut-game-server.x86_64
godot --headless --path apps/client --export-release Windows 'build/windows/DLUT Online.exe'
...
```sh
DO_SERVER_URL=http://127.0.0.1:8060 godot --headless --path apps/client --script tests/enet_protocol.gd
DO_SERVER_URL=http://127.0.0.1:8060 godot --headless --path apps/client --script tests/campus_travel.gd
DO_SERVER_URL=http://127.0.0.1:8060 godot --headless --path apps/client --script tests/campus_transfer.gd
DO_SERVER_URL=http://127.0.0.1:8060 godot --headless --path apps/client --script tests/player_network.gd
godot --headless --path apps/client --script tests/server_physics.gd
```
