...
### Requirement: 网站账号入口
网站 SHALL 提供邮箱密码登录、注册、验证邮件重发和退出。注册包含邮箱、唯一用户名、显示名、密码及确认密码，注册后提示先完成邮箱验证。导航从左到右为下载客户端文字链接按钮、单个“登录 / 注册”入口；该入口指向 /login，登录页保留切换注册的链接；登录后展示账号及退出。小屏幕表单和导航 SHALL 可用，不增加字间距。
...
#### Scenario: 网站登录
- **WHEN** 用户提交有效账号或退出
- **THEN** 登录后展示账号显示名，退出后恢复单个“登录 / 注册”入口；密码及 token 不写入 URL
...
### Requirement: 轻量登录首屏
客户端 SHALL 先显示原生登录封面，账号认证成功前不得加载校园模型、校园地图数据或创建校园碰撞。密码认证或保存会话刷新成功后 SHALL 异步加载默认校园，加载完成并收到游戏服 welcome 后自动进入，无额外继续按钮。

#### Scenario: 未登录启动
- **WHEN** 客户端启动且尚未认证成功
- **THEN** 仅显示登录封面，校园场景资源未加载，场景树中没有校园模型、玩家或地图控件

#### Scenario: 登录后加载
- **WHEN** PocketBase 密码认证或 auth-refresh 成功
...
#### Scenario: 返回封面
- **WHEN** 玩家退出登录或认证失效
- **THEN** 卸载校园场景并返回轻量登录封面，再次认证成功后重新加载校园
