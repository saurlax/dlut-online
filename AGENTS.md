...
- 当前建设可行走的校园世界、游客身份和 WebSocket 玩家同步；室内必须先具备实景照片或有效平面数据再建设，不自行增加活动、战斗、任务、奖励或持久账号功能。
...
- 不引入外部前端框架或自定义 HTML 产品 UI。JavaScriptBridge 仅允许浏览器地址读取与游客 sessionStorage 存储适配；联网使用 Godot 原生 WebSocketPeer。
...
- 三校区 Web 资源在导出时按整百米坐标对齐的 100 m × 100 m 网格生成细节包，建筑完整归属并跨格引用，材质共享。基础地面、建筑轮廓和通行碰撞先加载，附近细节渐进下载；编辑器及桌面保留完整模型，不手工维护第二套源模型。
