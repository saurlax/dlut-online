...
- 建筑、道路、广场、水域、绿地等平面基础数据统一优先采用 OSM，使用 WGS84 与同一校区原点生成局部 X 向东、Z 向南坐标；禁止按要素类型分别叠加旧官网质心平移。保留官方 Feature ID 与 OSM 对象 ID 的对应及原始来源档案，官网斜视交互 bound 不作为建筑地面轮廓。建筑应有独立可替换的模型；官网俯视图只用于配准核对及有证据的局部边框精修，记录控制点、转换、适用范围与误差，不把屋顶外沿当成墙脚，不将拟合残差声称为实测精度。
