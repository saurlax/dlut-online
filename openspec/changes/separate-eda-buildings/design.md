...
- 编辑器 TSCN 保留全部 Blender 建筑实例；运行时二进制场景只保留 `Feature_<id>` 占位节点，客户端在 220 米内异步加载建筑并在 300 米外卸载。服务端继续使用离线生成的全校区碰撞场景。
...
- 距离加载降低进入场景时的峰值内存，但接近高精度建筑时仍会产生单栋资源和碰撞的加载开销；后续精修需继续控制单栋面数。
