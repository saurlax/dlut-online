...
func check() -> void:
	...
	for label in ["client", "server"]:
		...
		if label == "client":
			...
			if not verify_reference(world): failures += 1
		...
		await physics_frame
		await physics_frame
		# Probe the declared source-road segment across the former duplicate volume.
		for x in range(int(walk_limits().x)+1,int(walk_limits().y)+2,2):
			...
			if hit.is_empty() or absf(hit.position.y-(y+0.12))>0.05:
				push_error("Reference road blocked or unsupported: %s x=%s" % [label,x])
				failures += 1
		if failures == 0:
			...
			for direction in [-1.0,1.0]:
				var start_x := walk_limits().y if direction<0 else walk_limits().x
				...
				for frame in 180:
					await physics_frame
					movement.step(body,Vector2(direction,direction*(road_z(start_x+1.0)-road_z(start_x))).normalized(),false,false,1.0/60.0,start)
		...
		world.queue_free()
		await process_frame
	print("REFERENCE ROAD ","PASS" if failures==0 else "FAIL",": client/server road samples and bidirectional walks; failures=",failures)
